...
BANNER_DB_CHAT   = int(os.environ.get("BANNER_DB_CHAT", ""))     # Channel ID used as banner DB
AIRING_PAGE_SIZE = int(os.environ.get("AIRING_PAGE_SIZE", ""))   # entries per airing page
...
NYAA_POLL_INTERVAL = int(os.environ.get("NYAA_POLL_INTERVAL", "60"))  # seconds
...
class Database:
    def __init__(self, uri: str):
        ...
        self.banners = self.db["banners"]   # Banner DB: {name, msg_id}
    ...
    async def init_indexes(self):
        ...
        await self.banners.create_index("name", unique=True)
...
class BannerDB:
    """
    Bot-token-safe Banner storage.

    Stores photos in a Telegram channel (BANNER_DB_CHAT) and indexes them
    in MongoDB collection "banners": { name: str, msg_id: int }

    ✅ NEVER uses get_chat_history / iter_history.
    ✅ Uses only send_photo() and get_messages() — fully bot-compatible.
    """

    BANNERS_PER_PAGE = 6   # 3 columns × 2 rows

    def __init__(self, db: "Database"):
        self.db = db

    # ── Save ──────────────────────────────────────────────────────────────
    async def save(self, app: "Client", name: str, photo_file_id: str) -> dict:
        """
        Upload photo to DB channel, upsert {name, msg_id} into MongoDB.
        ...
        try:
            ...
            sent    = await app.send_photo(
                BANNER_DB_CHAT, photo=photo_file_id, caption=name_lc
            )
            # Upsert into MongoDB
            existing = await self.db.banners.find_one({"name": name_lc})
            await self.db.banners.update_one(
                {"name": name_lc},
                {"$set": {"name": name_lc, "msg_id": sent.id}},
                upsert=True,
            )
            was_updated = existing is not None
            logger.info("BannerDB: saved '%s' msg_id=%d updated=%s", name_lc, sent.id, was_updated)
    ...
    # ── Fetch single photo ────────────────────────────────────────────────
    async def fetch_photo(self, app: "Client", msg_id: int):
        """
        Fetch a photo message from the DB channel by msg_id.
        Uses get_messages() — one targeted fetch, fully bot-safe.
        """
        ...
        try:
            ...
            logger.error("BannerDB.fetch_photo error msg_id=%d: %s", msg_id, exc)
            return None
    ...
    async def search(self, query: str = "") -> list:
        ...
        Empty query returns all banners sorted A→Z.
        Returns list of {name, msg_id} dicts.
        """
        filt = {}
        if query:
            import re as _re
            filt = {"name": {"$regex": _re.escape(query.lower()), "$options": "i"}}
    ...
    @staticmethod
    def banner_keyboard(
        banners: list, page: int, total_pages: int, query: str = ""
    ) -> InlineKeyboardMarkup:
        """Grid: 3 buttons/row (banner names), then pagination row."""
        q_enc = query[:30]
        rows  = []

        for i in range(0, len(banners), 3):
            chunk = banners[i : i + 3]
            rows.append([
                InlineKeyboardButton(
                    b["name"][:20],
                    callback_data=f"banner:show:{b['msg_id']}",
                )
                for b in chunk
            ])
        ...
        if total_pages > 1:
            nav = [
                InlineKeyboardButton(
                    "◀",
                    callback_data=f"banner:page:{page - 1}:{q_enc}" if page > 0 else "noop",
                ),
                InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="noop"),
                InlineKeyboardButton(
                    "▶",
                    callback_data=f"banner:page:{page + 1}:{q_enc}" if page < total_pages - 1 else "noop",
                ),
            ]
            rows.append(nav)
...
class AnimeBot:
    ...
    async def handle_callback(self, _, query: CallbackQuery):
        ...
        if data == "menu:back":
            ...
            # banner:show:<message_id>     — send the banner photo
            # banner:page:<page>:<query>   — paginate banner list
            ...
            if baction == "close":
                ...
                try:
                    tg_msg = await self.banner_db.fetch_photo(self.app, msg_id)
                    if tg_msg and tg_msg.photo:
                        ...
                        await query.answer("❌ Banner photo not found.", show_alert=True)
                except Exception as exc:
                    logger.error("Banner show error: %s", exc)
                    await query.answer("❌ Failed to retrieve banner.", show_alert=True)

            elif baction == "page":
                bpage  = int(bparts[2]) if len(bparts) > 2 and bparts[2].lstrip("-").isdigit() else 0
                bquery = bparts[3] if len(bparts) > 3 else ""
                await query.answer(f"Page {bpage + 1}…")
                banners = await self.banner_db.search(bquery)
                page_banners, total_pages = self.banner_db.paginate(banners, bpage)
                if not page_banners:
                    await query.answer("No banners found.", show_alert=True)
                    return
                kb  = self.banner_db.banner_keyboard(page_banners, bpage, total_pages, bquery)
                hdr = "🖼 <b>Banners</b>"
                if bquery:
                    hdr += f" — <i>{bquery}</i>"
                hdr += f"\n<i>{len(banners)} total</i>"
                await self.safe_edit(query, hdr, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
            ...
            # srch:cancel               — dismiss
            # srch:<source>:<query>     — run torrent search via AnimeTosho
            parts    = data.split(":", 2)
            ...
            if s_action == "cancel":
                ...
                if not s_query:
                    ...
                    return
                try:
                    await self._do_torrent_search(query, s_action, s_query)
                except Exception as exc:
    ...
    async def _auto_reply_naruto(self, _, msg: Message):
        """
        Auto-trigger Gemini when user replies to a bot message.
        The replied-to message must be FROM this bot.
        Supports both private and group chats.
        """
        if not await self._auth(msg):
            return
        # Only fire when replying to the bot itself
        ...
        try:
            ...
            return
        # Don't double-trigger if message already starts with "naruto"
        raw = (msg.text or msg.caption or "").strip()
        if re.match(r"(?i)^naruto\s*", raw):
            return
        # Use full message text as the prompt
        prompt = raw
        if not prompt:
            return
        thinking = await msg.reply("🍥 <i>Thinking, dattebayo…</i>", parse_mode=enums.ParseMode.HTML)
        uid = msg.from_user.id if msg.from_user else msg.chat.id
        reply_text = await self.gemini.chat(uid, prompt)
        try:
            await thinking.delete()
        except Exception:
            pass
        if len(reply_text) <= TG_MESSAGE_LIMIT:
            await msg.reply(reply_text, parse_mode=enums.ParseMode.HTML,
                            disable_web_page_preview=True)
        else:
            chunks, cur = [], ""
    ...
    async def cmd_save_banner(self, _, msg: Message):
        """
        /save_banner <name>  — reply to a JPG/PNG image to save it.

        • Name is stored lowercase (case-insensitive lookups).
        • If the same name exists → updates the stored image.
        • Uses send_photo() + get_messages() only. No get_chat_history.
        """
        if not await self._auth(msg):
            return
        if not await self._is_group_admin(msg) and not await self._is_bot_admin(msg):
        ...
        if not BANNER_DB_CHAT:
            await msg.reply(
                ...
                "Set <code>BANNER_DB_CHAT</code> env var to a channel ID where bot is admin.",
                parse_mode=enums.ParseMode.HTML,
            )
            return
        ...
        if not raw_name:
            await msg.reply(
                "ℹ️ <b>Usage:</b> <code>/save_banner &lt;name&gt;</code>\n"
                "Reply to a JPG/PNG image with this command.\n\n"
                "<b>Example:</b> <code>/save_banner welcome</code>",
                parse_mode=enums.ParseMode.HTML,
            )
            return

        # Always store lowercase for consistent lookups
        ...
        if replied:
            photo = replied.photo
            if not photo and replied.document:
        ...
        if not photo:
            await msg.reply(
                "❌ Please <b>reply to a JPG/PNG image</b> with this command.\n\n"
                f"<i>Send the image first, then reply with:</i>\n"
                f"<code>/save_banner {name}</code>",
                parse_mode=enums.ParseMode.HTML,
            )
            return
        ...
        wait    = await msg.reply(
            f"⏳ Saving banner <b>{name}</b>…", parse_mode=enums.ParseMode.HTML
        )
        result = await self.banner_db.save(self.app, name, file_id)
        try:
            await wait.delete()
        except Exception:
            pass
        ...
        if result.get("ok"):
            if result.get("updated"):
                await msg.reply(
                    f'♻️ Banner updated: "<b>{name}</b>"',
                    parse_mode=enums.ParseMode.HTML,
                )
            else:
                await msg.reply(
                    f'✅ Banner saved as "<b>{name}</b>"',
                    parse_mode=enums.ParseMode.HTML,
                )
        else:
            err = result.get("error", "unknown error")
            logger.error("cmd_save_banner failed for '%s': %s", name, err)
            await msg.reply(
                "❌ <b>Failed to save banner.</b>\n"
                "Make sure the bot is admin of the DB channel.\n"
                f"<i>Error: {err}</i>",
                parse_mode=enums.ParseMode.HTML,
            )
    ...
    async def cmd_banner(self, _, msg: Message):
        """
        /banner [name]  — search banners in DB channel by name.
        No name → show all banners.  Results in paginated button grid.
        """
        if not await self._auth(msg):
            return
        if not BANNER_DB_CHAT:
        ...
        query_str = " ".join(msg.command[1:]).strip()
        wait = await msg.reply("🔍 Searching banners…", parse_mode=enums.ParseMode.HTML)

        banners = await self.banner_db.search(query_str)
        try:
            await wait.delete()
        except Exception:
            pass
        ...
        if not banners:
            await msg.reply(
                f"❌ No banners found" + (f" for <b>{query_str}</b>" if query_str else "") + ".",
                parse_mode=enums.ParseMode.HTML,
            )
            return

        page_banners, total_pages = self.banner_db.paginate(banners, 0)
        kb  = self.banner_db.banner_keyboard(page_banners, 0, total_pages, query_str)
        hdr = f"🖼 <b>Banners</b>"
        if query_str:
            hdr += f" — <i>{query_str}</i>"
        hdr += f"\n<i>{len(banners)} result(s)</i>"
        await msg.reply(hdr, reply_markup=kb, parse_mode=enums.ParseMode.HTML)

    # ── /banner_list ─────────────────────────────────────────────────────
    async def cmd_banner_list(self, _, msg: Message):
        """
        /banner_list — show ALL saved banners as a paginated button grid.
        Each button shows the banner name; tapping it sends the image.
        No search filter — shows everything.
        """
        if not await self._auth(msg):
            return
        if not BANNER_DB_CHAT:
        ...
        wait = await msg.reply("📂 Loading all banners…", parse_mode=enums.ParseMode.HTML)
        # MongoDB search with empty query → returns all banners sorted A→Z
        banners = await self.banner_db.search("")
        try:
            await wait.delete()
        except Exception:
            pass
        ...
        if not banners:
            await msg.reply(
                ...
                "<i>Save one with:</i> <code>/save_banner &lt;name&gt;</code>",
                parse_mode=enums.ParseMode.HTML,
            )
            return

        page_banners, total_pages = self.banner_db.paginate(banners, 0)
        kb = self.banner_db.banner_keyboard(page_banners, 0, total_pages, "")
        count = len(banners)
        hdr = (
            f"🖼 <b>All Saved Banners</b>\n"
            f"<i>{count} banner{'s' if count != 1 else ''} — tap any to send</i>"
        )
        await msg.reply(hdr, reply_markup=kb, parse_mode=enums.ParseMode.HTML)
    ...
    async def _do_torrent_search(self, query_cb, source_key: str, anime_query: str):
        """
        Search AnimeTosho for the given anime query filtered by uploader.
        Build a Telegraph page with results and send the link.

        source_key: "varyg" | "toons" | "subsp" | "all"
        """
        ...
        sources = SOURCE_MAP.get(source_key, SOURCE_MAP["all"])
        all_results = []

        for src_cfg in sources:
            try:
                params = {"t": "search", "q": src_cfg["q"], "limit": 30}
                async with httpx.AsyncClient(timeout=20) as client:
                    ...
                    for item in data:
                        ...
                        if not title or not filt(title):
                            continue
                        nyaa_id = item.get("nyaa_id") or item.get("id", "")
                        view_url = item.get("link") or (
                            f"https://nyaa.si/view/{nyaa_id}" if nyaa_id else "#"
                        )
                ...
                logger.warning("Torrent search error for '%s': %s", src_cfg["q"], exc)
        ...
        if not all_results:
            await query_cb.message.reply(
                f"❌ No results found for <b>{anime_query}</b> via {source_key}.",
                parse_mode=enums.ParseMode.HTML,
            )
            return

        # ── Build and post Telegraph page ──────────────────────────────────
        ...
        text = (
            f"🎌 <b>Torrent Results</b>\n\n"
            f"Query: <i>{anime_query}</i>\n"
            f"Source: {src_label}\n"
            f"Found: <b>{result_count}</b> result(s)\n\n"
        )
        kb = None
        if tg_link:
            text += f'📄 <a href="{tg_link}">View full results on Telegraph</a>'
        else:
            # Telegraph failed — send inline text (first 5 results)
            for res in all_results[:5]:
                text += (
                    f"\n{res['icon']} <b>{res['title'][:60]}</b>\n"
                    f"  📦 {res['size']}"
                )
                if res["torrent_url"]:
                    text += f' · <a href="{res["torrent_url"]}">DL</a>'
                if res["view_url"] and res["view_url"] != "#":
                    text += f' · <a href="{res["view_url"]}">View</a>'
                text += "\n"
            if result_count > 5:
                text += f"<i>…and {result_count - 5} more</i>"
    ...
    async def _post_to_telegraph(self, title: str, results: list, source_key: str) -> Optional[str]:
        """
        Post search results to Telegraph API and return the page URL.
        Returns None if the post fails.
        """
        TELEGRAPH_API = "https://api.telegra.ph"

        # Build Telegraph HTML content
        html_parts = [f"<h3>Search: {title}</h3>"]
        label_map = {
            "varyg": "🔴 varyg1001",
            "toons": "🟡 ToonsHub",
            "subsp": "🟢 SubsPlease",
        ...
        html_parts.append(f"<p>Source: {label_map.get(source_key, source_key)} | {len(results)} results</p>")
        ...
        for i, r in enumerate(results, 1):
            ...
            if r.get("torrent_url"):
                links.append(f'<a href="{r["torrent_url"]}">⬇ Download</a>')
            if r.get("view_url") and r["view_url"] != "#":
                links.append(f'<a href="{r["view_url"]}">🔗 View</a>')
            link_str = " · ".join(links) if links else ""
            html_parts.append(
                f"<p><b>{i}. {icon} {title_text}</b><br/>"
                f"📦 {size}"
                + (f"<br/>{link_str}" if link_str else "")
                + "</p>"
            )
        ...
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                # Create anonymous Telegraph page
                page_payload = {
    ...
    async def handle_inline(self, _, query):
        """
        @botusername <query> — search saved banners, return as inline photo results.

        Uses MongoDB for search (no get_chat_history).
        Fetches each photo via get_messages() — one targeted lookup per result.
        Pyrogram 2.x accepts file_id as photo_url for cached Telegram files.
        """
        if not BANNER_DB_CHAT:
        ...
        # Pure MongoDB lookup — no channel history traversal
        banners = await self.banner_db.search(q)
        results = []

        for b in banners[:50]:   # Telegram inline query max: 50 results
            msg_id = b.get("msg_id") or b.get("message_id")
            name   = b.get("name", "banner")
            if not msg_id:
                continue
            try:
                # Single-message fetch — get_messages is fully bot-safe
                tg_msg = await self.app.get_messages(BANNER_DB_CHAT, msg_id)
                if not tg_msg or getattr(tg_msg, "empty", False):
                    continue
                if tg_msg.photo:
                    fid = tg_msg.photo.file_id
                elif tg_msg.document and (tg_msg.document.mime_type or "").startswith("image/"):
                    fid = tg_msg.document.file_id
                else:
                    continue
                results.append(
                    InlineQueryResultPhoto(
                ...
                logger.debug("Inline banner skip msg_id=%s: %s", msg_id, exc)
    ...
    def _register(self, app: Client):
        ...
        # ── NEW: Reply-to-bot triggers Gemini ──────────────────────────────
        app.on_message(
            filters.reply & filters.text & ~filters.command([])
        )(self._auto_reply_naruto)
        # ── Bot admin commands ──────────────────────────────────────────────
