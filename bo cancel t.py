...
class NyaaScraper:
    ...
    def _parse_subsplease_rss(xml: str) -> list:
        ...
        try:
            ...
            for item in soup.find_all("item"):
                ...
                # Fallback: subsplease sometimes puts .torrent link in <link> tag
                if not torrent_url:
                    link_tag = item.find("link")
                    if link_tag:
                        torrent_url = link_tag.get("href") or link_tag.get_text(strip=True) or ""
    ...
    def format_entry(e: dict) -> str:
        ...
        # Build view link — convert AnimeTosho view URLs to nyaa.si links where possible
        if uploader == "subsplease":
            view_line = f"\n🌐 <a href=\"{view_url}\">subsplease.org</a>"
        elif view_url:
            # AnimeTosho JSON returns animetosho.org links; convert to nyaa.si/view/<id>
            import re as _re
            nyaa_m = _re.search(r"nyaa_id=(\d+)|/view/(\d+)", view_url)
            if nyaa_m:
                nyaa_id = nyaa_m.group(1) or nyaa_m.group(2)
                nyaa_view = f"https://nyaa.si/view/{nyaa_id}"
            else:
                nyaa_view = view_url
