...
class NyaaScraper:
    ...
    Polls three 1080p torrent sources:
      1. varyg1001  — AnimeTosho API (mirrors nyaa.si, no Cloudflare block)
      2. ToonsHub   — AnimeTosho API (search [ToonsHub] 1080)
      3. SubsPlease — https://subsplease.org/rss/?t&r=1080 (SubsPlease own RSS)

    AnimeTosho (feed.animetosho.org) mirrors all nyaa.si releases and exposes a
    free JSON/RSS API — no API key, no Cloudflare, no 504s.

    Uses pubDate watermarking in MongoDB. First run seeds silently, no backlog.
    """

    # AnimeTosho JSON API — mirrors nyaa.si without Cloudflare issues
    # /json endpoint returns JSON array; /api returns XML (torznab)
    ANIMETOSHO_API = "https://feed.animetosho.org/json"
    ANIMETOSHO_SOURCES = {
        "varyg1001": {"q": "varyg1001 1080"},
        "ToonsHub":  {"q": "[ToonsHub] 1080"},
    }
    # SubsPlease own RSS (their site, reliable 200)
    ...
    async def fetch_new(self) -> list:
        ...
        # ── AnimeTosho API (mirrors nyaa.si — no Cloudflare block) ───────
        for source, params in self.ANIMETOSHO_SOURCES.items():
            entries = await self._fetch_animetosho(source, params["q"])
            if entries:
    ...
    async def _fetch_animetosho(self, source: str, q: str) -> list:
        """
        Fetch from AnimeTosho JSON API — mirrors nyaa.si without Cloudflare.
        Endpoint: https://feed.animetosho.org/json?t=search&q=<query>&limit=100
        Returns a JSON array of release objects.
        """
        url = self.ANIMETOSHO_API
        ...
        for attempt in range(3):
            try:
                ...
                if r.status_code != 200:
                    logger.warning("AnimeTosho [%s] status %d body: %s", source, r.status_code, r.text[:200])
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                raw = r.text.strip()
                if not raw:
                    logger.warning("AnimeTosho [%s] empty response body", source)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                data = r.json()
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_animetosho_json, data, source
                )
                logger.info("AnimeTosho [%s] got %d entries", source, len(entries))
                return entries
            except Exception as exc:
                logger.warning("AnimeTosho [%s] attempt %d error: %s | body: %s",
                               source, attempt + 1, exc,
                               r.text[:200] if "r" in dir() else "n/a")
                if attempt < 2:
                    await asyncio.sleep(10)
        return []
    ...
    def _parse_animetosho_json(data: list, source: str) -> list:
        """
        Parse AnimeTosho JSON API response.
        Each item has: title, link (nyaa view url), timestamp (unix),
        torrent_url, total_size (bytes), nyaa_id.
        """
        entries = []
        try:
            if not isinstance(data, list):
                # API may wrap in {"items": [...]}
            ...
            for item in data:
                ...
                if "1080" not in title:
                    continue
                # ToonsHub: must have [ToonsHub] tag
                if source == "ToonsHub" and "[ToonsHub]" not in title:
                    continue
                # varyg1001: uploaded by varyg1001 — check nyaa_submitter or title
                # AnimeTosho doesn't expose uploader directly in JSON search,
                # so for varyg1001 we rely on the search query already filtering by name
                # Timestamp
