...
class ScheduleProcessor:
    ...
    def process(entries: list) -> dict:
        ...
        Each version slot carries its own episode number and date so
        anime_block() can display them accurately.
        """
        # key = (slug, day, version_group)
        # version_group: "raw_sub" for raw/sub entries, "dub" for dub entries
        # This ensures Dub on Saturday never gets merged with Sub on Wednesday.
        merged: dict = {}
    ...
    def day_pages(day: str, anime_list: list, max_len: int = 3800) -> list:
        """
        Build paginated schedule for a day with two clear sections:
          Raw & Sub  - shows with raw or sub airing today
          Dubbed     - shows with dub airing today (separate section, own day)
        """
        if not anime_list:
            return [f"<b>\U0001f4c5 {day}</b>\n\nNo anime scheduled."]

        rawsub_entries = [a for a in anime_list if a.get("raw_time") or a.get("sub_time")]
        dub_entries    = [a for a in anime_list if a.get("dub_time")]

        rawsub_blocks = [b for b in (ScheduleProcessor.anime_block_rawsub(a) for a in rawsub_entries) if b]
        dub_blocks    = [b for b in (ScheduleProcessor.anime_block_dub(a)    for a in dub_entries)    if b]

        if not rawsub_blocks and not dub_blocks:
            return [f"<b>\U0001f4c5 {day}</b>\n\nNo anime scheduled."]

        total_shows = len(set(
            a["slug"] for a in anime_list
            if a.get("raw_time") or a.get("sub_time") or a.get("dub_time")
        ))
        base_header = f"<b>\U0001f4c5 {day}</b> \u2014 {total_shows} show(s)"
        ...
        if rawsub_blocks:
            parts.append(
                f"\U0001f4fa <b>Raw &amp; Sub</b> \u2014 {len(rawsub_blocks)} show(s)\n\n"
                + "\n\n".join(rawsub_blocks)
            )
        if dub_blocks:
            parts.append(
                f"\U0001f3ac <b>Dubbed</b> \u2014 {len(dub_blocks)} show(s)\n\n"
                + "\n\n".join(dub_blocks)
            )

        full_content = f"\n\n{DIVIDER}\n\n".join(parts)

        # If fits in one page, return immediately
        if len(base_header) + 2 + len(full_content) <= max_len:
            return [base_header + "\n\n" + full_content]

        # Paginate: build a flat list of items (section headers + blocks)
        flat: list = []
        if rawsub_blocks:
            flat.append(f"\U0001f4fa <b>Raw &amp; Sub</b> \u2014 {len(rawsub_blocks)} show(s)")
            flat.extend(rawsub_blocks)
        if dub_blocks:
            flat.append(DIVIDER)
            flat.append(f"\U0001f3ac <b>Dubbed</b> \u2014 {len(dub_blocks)} show(s)")
            flat.extend(dub_blocks)
        ...
        for item in flat:
            cost = len(item) + (2 if current_items else 0)
            if current_items and current_len + cost > max_len:
                pages.append(current_items)
                current_items = [item]
