    ...
    async def _send_reminder(self, job: dict):
        ...
        for chat_id in chats:
            ...
            if mode != "all" and mode != job["version"]:
                continue
            try:
                await self.app_ref.send_message(chat_id, text, parse_mode="html")
            except Exception as exc:
...
class AnimeBot:
    ...
    async def cmd_anime(self, _, msg: Message):
        ...
        if not query:
            await msg.reply("Usage: /anime &lt;name&gt;", parse_mode="html")
            return

        await msg.reply(f"🔍 Searching <i>{query}</i>…", parse_mode="html")
        ...
        if thumbnail:
            try:
                await msg.reply_photo(photo=thumbnail, caption=caption, parse_mode="html")
                return
            except Exception:
                pass
    ...
    async def cmd_broadcast(self, _, msg: Message):
        ...
        if not text:
            await msg.reply("Usage: /broadcast &lt;message&gt;", parse_mode="html")
            return
        chats = await self.db.all_subscribed_chats()
        sent = 0
        for cid in chats:
