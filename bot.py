        ...
        async for doc in self.chats.find({"topics": {"$exists": True}}):
            ...
            for tid, mode in topics.items():
                if mode == "rem":
                    ...
                    seen.add(cid)  # mark so Group 2 doesn't add a plain reminder

        # Group 2: subscribed chats with NO topic binding at all (plain reminders).
        # Skip any chat that has topics defined — those get topic-mode delivery only.
        async for doc in self.chats.find({"subscribed": True}):
            cid    = doc["chat_id"]
            topics = doc.get("topics") or {}
            # Skip if already added via Group 1 OR if chat has any topic bindings
            # (prevents nyaa-only chats from also receiving plain reminders)
            if cid in seen or topics:
                continue
