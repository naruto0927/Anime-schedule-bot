...
class NyaaScraper:
    ...
    async def fetch_new(self) -> list:
        ...
        # varyg: search AnimeTosho JSON for releases ending in -VARYG at 1080p
        varyg_entries = await self._fetch_animetosho_json("VARYG 1080", label="varyg1001")
        if varyg_entries:
            varyg_entries = [
                e for e in varyg_entries
                if "-VARYG" in e["title"].upper() and "1080" in e["title"]
            ]
            # Tag uploader as varyg1001 for display/config purposes
            for e in varyg_entries:
                e["uploader"] = "varyg1001"
        ...
        toonshub_entries = await self._fetch_animetosho_json("[ToonsHub] 1080", label="ToonsHub")
        if toonshub_entries:
    ...
    async def _fetch_animetosho_json(self, q: str, label: str = "search") -> list:
        """Fetch entries via AnimeTosho JSON search."""
        params = {"t": "search", "q": q, "limit": 100}
        for attempt in range(3):
            try:
                r = await self.client.get(self.ANIMETOSHO_JSON, params=params)
                if r.status_code == 429:
                    logger.warning("AnimeTosho JSON [%s] rate-limited, waiting 30s", label)
                    await asyncio.sleep(30)
                    continue
                if r.status_code != 200:
                    logger.warning("AnimeTosho JSON [%s] status %d", label, r.status_code)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                if not r.text.strip():
                    logger.warning("AnimeTosho JSON [%s] empty body", label)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                data = r.json()
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_animetosho_json, data, label
                )
                logger.info("AnimeTosho JSON [%s] got %d entries", label, len(entries))
                return entries
            except Exception as exc:
                logger.warning("AnimeTosho JSON [%s] attempt %d error: %s", label, attempt + 1, exc)
                if attempt < 2:
                    await asyncio.sleep(10)
        return []
...
class AnimeBot:
    ...
    async def handle_callback(self, _, query: CallbackQuery):
        ...
        if data == "menu:back":
            ...
            day = data.split(":", 1)[1]  # e.g. "monday"
            msg_text = await self.db.get_day_message(chat_id, day)
            if msg_text:
                text = (
                    f"📅 <b>{day.capitalize()} — Animes</b>\n\n"
                    f"{msg_text}"
                )
            else:
                text = (
                    f"📅 <b>{day.capitalize()} — Animes</b>\n\n"
                    f"<i>No list set yet.</i>\n\n"
                    f"Set one with:\n"
                    f"<code>/set{day} Your anime list here...</code>"
                )
            await self.safe_edit(
                query,
                text,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🗑 Clear", callback_data=f"assigned:clear:{day}")],
        ...
        elif data.startswith("assigned:clear:"):
            day = data.split(":", 2)[2]
            await self.db.clear_day_message(chat_id, day)
            await query.answer(f"🗑 {day.capitalize()} message cleared.")
            await self.safe_edit(
                query,
                "📋 <b>Animes Assigned</b>\n\nSelect a day to view its assigned anime list:",
                reply_markup=self._kb_assigned(),
