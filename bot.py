...
class NyaaScraper:
    ...
    Polls three 1080p torrent sources:
      1. varyg1001  — nyaa.si RSS (primary) → AnimeTosho JSON fallback
      2. ToonsHub   — nyaa.si RSS (primary) → AnimeTosho JSON fallback
      3. SubsPlease — subsplease.org RSS (always reliable)

    nyaa.si RSS is tried first; if it returns 5xx (Cloudflare block on this IP),
    AnimeTosho JSON is used as fallback so delivery continues.
    Uses pubDate watermarking in MongoDB. First run seeds silently, no backlog.
    """

    # nyaa.si RSS sources (primary)
    NYAA_RSS_SOURCES = {
        "varyg1001": "https://nyaa.si/user/varyg1001?page=rss&q=1080",
        "ToonsHub":  "https://nyaa.si/?page=rss&q=%5BToonsHub%5D+1080",
    }
    # AnimeTosho JSON fallback (used when nyaa.si is blocked)
    ANIMETOSHO_JSON = "https://feed.animetosho.org/json"
    ANIMETOSHO_FALLBACK = {
        "varyg1001": {"q": "VARYG 1080",       "title_filter": lambda t: "-VARYG" in t.upper()},
        "ToonsHub":  {"q": "[ToonsHub] 1080",   "title_filter": lambda t: "[ToonsHub]" in t},
    }
    # SubsPlease own RSS
    ...
    async def fetch_new(self) -> list:
        ...
        # varyg1001 + ToonsHub: nyaa.si RSS primary, AnimeTosho fallback
        for _src in ("varyg1001", "ToonsHub"):
            _entries = await self._fetch_nyaa_source(_src)
            if _entries:
                new_entries.extend(await self._filter_new(_src, _entries))

        # SubsPlease: their own RSS (always reliable)
    ...
    async def _fetch_nyaa_source(self, source: str) -> list:
        """
        Fetch a nyaa.si RSS source. On 5xx (Cloudflare block) falls back to
        AnimeTosho JSON automatically so delivery is never silently lost.
        """
        url = self.NYAA_RSS_SOURCES[source]
        # ── Primary: nyaa.si RSS ──────────────────────────────────────────
        for attempt in range(2):
            try:
                r = await self.client.get(url)
                if r.status_code == 429:
                    logger.warning("nyaa.si [%s] rate-limited, waiting 20s", source)
                    await asyncio.sleep(20)
                    continue
                if r.status_code >= 500:
                    logger.warning("nyaa.si [%s] %d — using AnimeTosho fallback", source, r.status_code)
                    break  # go to fallback
                if r.status_code != 200:
                    logger.warning("nyaa.si [%s] unexpected status %d", source, r.status_code)
                    break
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_nyaa_rss, r.text, source
                )
                logger.info("nyaa.si RSS [%s] got %d entries", source, len(entries))
                return entries
            except Exception as exc:
                logger.warning("nyaa.si RSS [%s] attempt %d error: %s", source, attempt + 1, exc)
                if attempt == 0:
                    await asyncio.sleep(5)

        # ── Fallback: AnimeTosho JSON ─────────────────────────────────────
        fb = self.ANIMETOSHO_FALLBACK.get(source)
        if not fb:
            return []
        title_filter = fb["title_filter"]
        params = {"t": "search", "q": fb["q"], "limit": 100}
        for attempt in range(2):
            try:
                r = await self.client.get(self.ANIMETOSHO_JSON, params=params)
                if r.status_code != 200 or not r.text.strip():
                    logger.warning("AnimeTosho fallback [%s] status %d", source, r.status_code)
                    if attempt == 0:
                        await asyncio.sleep(10)
                    continue
                data = r.json()
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_animetosho_json, data, source, title_filter
                )
                logger.info("AnimeTosho fallback [%s] got %d entries", source, len(entries))
                return entries
            except Exception as exc:
                logger.warning("AnimeTosho fallback [%s] attempt %d error: %s", source, attempt + 1, exc)
                if attempt == 0:
                    await asyncio.sleep(10)
        return []

    @staticmethod
    def _parse_nyaa_rss(xml: str, source: str) -> list:
        """
        Parse nyaa.si RSS2 feed.
        Fields: <title>, <pubDate>, <nyaa:size>, <link> (view), <enclosure> (torrent).
        """
        ...
        try:
            ...
            for item in soup.find_all("item"):
                ...
                if source == "ToonsHub" and "[ToonsHub]" not in title:
                    continue
                if source == "varyg1001" and "-VARYG" not in title.upper():
                    continue
                ...
                try:
                    ...
                    continue
                # View URL from <guid> (nyaa.si/view/<id>)
                guid_tag = item.find("guid")
                view_url = guid_tag.get_text(strip=True) if guid_tag else ""
                # Torrent from <enclosure url> or derive from guid
            ...
            logger.error("nyaa.si RSS parse [%s] error: %s", source, exc)
        return entries

    @staticmethod
    def _parse_animetosho_json(data: list, source: str, title_filter=None) -> list:
...
class AnimeBot:
    ...
    async def cmd_set_day(self, _, msg: Message):
        ...
        # Use raw message text to preserve newlines — msg.command splits on whitespace
        raw = msg.text or msg.caption or ""
        # Strip the command prefix (e.g. "/setmonday " or "/setmonday@BotName ")
        prefix_end = raw.find(" ")
        text = raw[prefix_end:].strip() if prefix_end != -1 else ""
        if not text:
            await msg.reply(
                f"Usage: <code>/{cmd} Your anime list here...</code>\n"
                f"Newlines are preserved.",
                parse_mode=enums.ParseMode.HTML,
            )
            return
        await self.db.set_day_message(msg.chat.id, day, text)
        await msg.reply(
    ...
    def _kb_assigned(back: str = "menu:back") -> InlineKeyboardMarkup:
        ...
        rows = [[InlineKeyboardButton(d, callback_data=f"assigned:{d.lower()}")] for d in days]
    ...
    def _paginate_assigned(header: str, body: str) -> list:
        """Split a day message into pages that fit Telegram's message limit."""
        lines = body.split("\n")
        pages = []
        current = header + "\n\n"
        for line in lines:
    ...
    async def _show_assigned_day(self, query, chat_id: int, day: str, pg: int):
        ...
        if not msg_text:
            body = (
                f"{header}\n\n<i>No list set yet.</i>\n\n"
                f"Set one with: <code>/set{day} Your list...</code>"
            )
            await self.safe_edit(
                query, body,
                reply_markup=InlineKeyboardMarkup([
            ...
            return
        pages = self._paginate_assigned(header, msg_text)
        total = len(pages)
