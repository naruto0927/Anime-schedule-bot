...
class NyaaScraper:
    """
    Polls three 1080p torrent sources via AnimeTosho (nyaa.si mirror) + SubsPlease:
      1. varyg1001  — AnimeTosho RSS latest feed, filter by <nyaa:submitter>varyg1001
      2. ToonsHub   — AnimeTosho JSON search for "[ToonsHub] 1080" in title
      3. SubsPlease — https://subsplease.org/rss/?t&r=1080 (SubsPlease own RSS)

    AnimeTosho mirrors all nyaa.si releases — no Cloudflare, no 504s.
    Uses pubDate watermarking in MongoDB. First run seeds silently, no backlog.
    """

    # AnimeTosho JSON search — works for title-tagged uploaders like [ToonsHub]
    ANIMETOSHO_JSON = "https://feed.animetosho.org/json"
    # AnimeTosho RSS latest feed — includes <nyaa:submitter> for uploader filtering
    ...
    async def fetch_new(self) -> list:
        ...
        # varyg1001: use AnimeTosho RSS latest feed, filter by <nyaa:submitter>
        varyg_entries = await self._fetch_animetosho_rss_uploader("varyg1001")
        if varyg_entries:
            new_entries.extend(await self._filter_new("varyg1001", varyg_entries))

        # ToonsHub: use AnimeTosho JSON search (title contains [ToonsHub])
        toonshub_entries = await self._fetch_animetosho_json("[ToonsHub] 1080")
        if toonshub_entries:
            # Extra guard: ensure [ToonsHub] and 1080 in title
            toonshub_entries = [
                e for e in toonshub_entries
                if "[ToonsHub]" in e["title"] and "1080" in e["title"]
            ]
            new_entries.extend(await self._filter_new("ToonsHub", toonshub_entries))

        # SubsPlease: their own RSS
    ...
    async def _fetch_animetosho_json(self, q: str) -> list:
        """Fetch ToonsHub entries via AnimeTosho JSON search."""
        params = {"t": "search", "q": q, "limit": 100}
        for attempt in range(3):
            try:
                r = await self.client.get(self.ANIMETOSHO_JSON, params=params)
                if r.status_code == 429:
                    logger.warning("AnimeTosho JSON rate-limited, waiting 30s")
                    await asyncio.sleep(30)
                    continue
                if r.status_code != 200:
                    logger.warning("AnimeTosho JSON status %d", r.status_code)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                if not r.text.strip():
                    logger.warning("AnimeTosho JSON empty body")
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                data = r.json()
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_animetosho_json, data, "ToonsHub"
                )
                logger.info("AnimeTosho JSON [ToonsHub] got %d entries", len(entries))
                return entries
            except Exception as exc:
                logger.warning("AnimeTosho JSON attempt %d error: %s", attempt + 1, exc)
                if attempt < 2:
                    await asyncio.sleep(10)
        return []

    async def _fetch_animetosho_rss_uploader(self, uploader: str) -> list:
        """
        Fetch AnimeTosho RSS and filter by <nyaa:submitter> == uploader.
        This is the only reliable way to get releases by uploader name when
        the uploader name is not in the release title (e.g. varyg1001).
        """
        params = {"t": "search", "q": uploader, "limit": 100}
        for attempt in range(3):
            try:
                r = await self.client.get(self.ANIMETOSHO_RSS, params=params)
                if r.status_code == 429:
                    logger.warning("AnimeTosho RSS rate-limited, waiting 30s")
                    await asyncio.sleep(30)
                    continue
                if r.status_code != 200:
                    logger.warning("AnimeTosho RSS status %d", r.status_code)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_animetosho_rss, r.text, uploader
                )
                logger.info("AnimeTosho RSS [%s] got %d entries", uploader, len(entries))
                return entries
            except Exception as exc:
                logger.warning("AnimeTosho RSS [%s] attempt %d error: %s", uploader, attempt + 1, exc)
                if attempt < 2:
                    await asyncio.sleep(10)
        return []

    @staticmethod
    def _parse_animetosho_rss(xml: str, uploader: str) -> list:
        """
        Parse AnimeTosho RSS2 feed, filtering by <nyaa:submitter> == uploader.
        Fields: <title>, <pubDate>, <nyaa:submitter>, <nyaa:size>,
                <link> (view URL), <enclosure url> (torrent download).
        """
        ...
        try:
            ...
            for item in soup.find_all("item"):
                submitter_tag = item.find("nyaa:submitter") or item.find("submitter")
                submitter = submitter_tag.get_text(strip=True) if submitter_tag else ""
                if submitter.lower() != uploader.lower():
                    continue
                ...
                view_url = link_tag.get_text(strip=True) if link_tag else ""
                if not view_url:
                    guid_tag = item.find("guid")
                    view_url = guid_tag.get_text(strip=True) if guid_tag else ""
                enc = item.find("enclosure")
                torrent_url = enc["url"] if enc and enc.get("url") else ""
                if not torrent_url:
                    import re as _re
                    m = _re.search(r"/view/(\d+)", view_url)
                    if m:
                        torrent_url = f"https://nyaa.si/download/{m.group(1)}.torrent"
                size_tag = item.find("nyaa:size") or item.find("size")
            ...
            logger.error("AnimeTosho RSS parse [%s] error: %s", uploader, exc)
        return entries

    @staticmethod
    def _parse_animetosho_json(data: list, source: str) -> list:
        """Parse AnimeTosho JSON search response."""
        entries = []
        try:
            ...
            for item in data:
                ...
                if not title or "1080" not in title:
                    continue
            ...
            logger.error("AnimeTosho JSON parse [%s] error: %s", source, exc)
        return entries

    async def _fetch_subsplease_rss(self) -> list:
        """Fetch SubsPlease 1080p RSS directly from subsplease.org (not nyaa.si)."""
        for attempt in range(3):
            try:
                r = await self.client.get(self.SUBSPLEASE_RSS)
                if r.status_code != 200:
                    logger.warning("SubsPlease RSS status %d", r.status_code)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_subsplease_rss, r.text
                )
    ...
    @staticmethod
    def _parse_subsplease_rss(xml: str) -> list:
        """Parse SubsPlease RSS feed. Uses <enclosure url> for torrent link."""
        from email.utils import parsedate_to_datetime
        entries = []
        try:
            try:
            ...
            logger.error("SubsPlease RSS parse error: %s", exc)
        return entries
    ...
    def format_entry(e: dict) -> str:
        ...
        # Build view link based on uploader, not URL content
        # (AnimeTosho JSON returns animetosho.org URLs, not nyaa.si)
        if uploader == "subsplease":
            view_line = f"\n🌐 <a href=\"{view_url}\">subsplease.org</a>"
        elif view_url:
            view_line = f"\n🔗 <a href=\"{view_url}\">View on Nyaa / AnimeTosho</a>"
        else:
            view_line = ""
        return (
            f"🎌 <b>{e['title']}</b>\n"
