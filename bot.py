...
def build_filter_kb(f: dict) -> InlineKeyboardMarkup:
...
class AnimeBot:
    ...
    async def handle_callback(self, _, query: CallbackQuery):
        ...
        if data == "menu:back":
            ...
            await query.edit_message_text(text, reply_markup=self._kb_main(),
                                          parse_mode=enums.ParseMode.HTML)
            ...
            schedule = await self._get_schedule(chat_id)
            f        = await self.db.get_filters(chat_id)
            text     = ScheduleProcessor.day_text(day, schedule.get(day, []), f)
            if len(text) > 4000:
                text = text[:3997] + "\u2026"
            await query.edit_message_text(
                text,
