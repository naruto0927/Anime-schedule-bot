...
class NyaaScraper:
    """
    Polls three 1080p torrent sources by scraping HTML / RSS pages directly:
      1. varyg1001  — https://nyaa.si/user/varyg1001  (HTML page scrape, q=1080)
      2. ToonsHub   — https://nyaa.si/?q=[ToonsHub]   (HTML page scrape, 1080 filter)
      3. SubsPlease — https://subsplease.org/rss/?t&r=1080  (SubsPlease own RSS)

    Uses pubDate / upload-date watermarking in MongoDB to detect genuinely new
    entries.  On first run the watermark is seeded silently so no backlog floods.
    """

    # nyaa.si HTML page URLs (not RSS — more reliable, no 504 issues)
    NYAA_SOURCES = {
        "varyg1001": "https://nyaa.si/user/varyg1001?f=0&c=0_0&q=1080&s=id&o=desc",
        "ToonsHub":  "https://nyaa.si/?f=0&c=0_0&q=%5BToonsHub%5D+1080&s=id&o=desc",
    }
    # SubsPlease own RSS (reliable 200, not nyaa.si)
    ...
    async def fetch_new(self) -> list:
        """
        Scrape all sources and return entries published after the stored watermark.
        First-run: seeds watermark silently so no backlog is sent on restart.
        """
        new_entries: list = []

        # ── nyaa.si HTML scrape (varyg1001 + ToonsHub) ───────────────────
        for source, url in self.NYAA_SOURCES.items():
            entries = await self._fetch_nyaa_html(url, source)
            if not entries:
                continue
    ...
    async def _fetch_nyaa_html(self, url: str, source: str) -> list:
        """Scrape nyaa.si torrent listing HTML page and return entry dicts."""
        for attempt in range(3):
            try:
                r = await self.client.get(url)
                if r.status_code == 429:
                    logger.warning("Nyaa HTML [%s] rate-limited, waiting 20s", source)
                    await asyncio.sleep(20)
                    continue
                if r.status_code != 200:
                    logger.warning("Nyaa HTML [%s] status %d", source, r.status_code)
                    if attempt < 2:
                        await asyncio.sleep(10)
                    continue
                entries = await asyncio.get_event_loop().run_in_executor(
                    None, self._parse_nyaa_html, r.text, source
                )
                logger.info("Nyaa HTML [%s] scraped %d entries", source, len(entries))
                return entries
            except Exception as exc:
                logger.warning("Nyaa HTML fetch [%s] attempt %d error: %s", source, attempt + 1, exc)
                if attempt < 2:
                    await asyncio.sleep(10)
        return []

    @staticmethod
    def _parse_nyaa_html(html: str, source: str) -> list:
        """
        Parse nyaa.si torrent listing page HTML.
        Table structure: category | name+link | size | date | seeders | leechers | downloads | actions
        Columns (0-indexed): 0=cat, 1=name, 2=size, 3=date, 4=S, 5=L, 6=D, 7=dl-links
        """
        ...
        try:
            ...
            if not table:
                logger.warning("Nyaa HTML [%s]: torrent-list table not found", source)
                return []
            for row in table.find("tbody").find_all("tr"):
                cells = row.find_all("td")
                if len(cells) < 4:
                    continue
                # Name cell (index 1): first <a> without class is the title link
                name_cell = cells[1]
                title_tag = name_cell.find("a", class_=False) or name_cell.find("a")
                if not title_tag:
                    continue
                title    = title_tag.get_text(strip=True)
                view_url = "https://nyaa.si" + title_tag.get("href", "")
                # Extract nyaa ID from href /view/<id>
                m = re.search(r"/view/(\d+)", view_url)
                nyaa_id  = m.group(1) if m else None
                torrent_url = f"https://nyaa.si/download/{nyaa_id}.torrent" if nyaa_id else ""
                uid = view_url
                # Size (index 2)
                size = cells[2].get_text(strip=True) if len(cells) > 2 else "?"
                # Date (index 3): data-timestamp unix epoch or text
                date_cell = cells[3]
                ts_tag = date_cell.find(attrs={"data-timestamp": True})
                if ts_tag:
                    try:
                        pub_dt = _dt.fromtimestamp(int(ts_tag["data-timestamp"]), tz=timezone.utc)
                    except Exception:
                        pub_dt = _dt.now(timezone.utc)
                else:
                    # Fallback: parse text like "2024-03-27 14:30"
                    try:
                        pub_dt = _dt.strptime(date_cell.get_text(strip=True), "%Y-%m-%d %H:%M").replace(tzinfo=timezone.utc)
                    except Exception:
                        pub_dt = _dt.now(timezone.utc)
                # Filters
            ...
            logger.error("Nyaa HTML parse [%s] error: %s", source, exc)
        return entries
