...
class AnimeScheduleScraper:
    ...
    def _coerce_entry(e: dict) -> Optional[dict]:
        ...
        slug = route or    logger.info("Health server on port %s", PORT)
    return runner
...
class AnimeScheduleScraper:
    ...
    Fetch strategy (attempted in order on each cache miss):
      1. Call the site's own JSON API endpoint with airType=all (no token
         needed for the unauthenticated public timetable).
      2. If the API returns a non-200/auth-error, fall back to fetching the
         SSR HTML page and extracting the __NEXT_DATA__ Next.js hydration
         payload or any inline window.__* JSON assignment.
      3. On complete failure, return [] and let the caller use the stale DB
         cache via get_cache_ignore_ttl() so the bot never crashes.

    Returned entry dicts use the same key set that ScheduleProcessor expects:
        route, title, episodeNumber, episodes,
        episodeDate, subDate, dubDate,
        imageVersionRoute, likes
    All datetime strings are ISO-8601 UTC.
    The animeschedule.net null sentinel "0001-01-01T00:00:00Z" is kept as-is;
    parse_dt() treats it as absent.
    """
    ...
    async def _fetch_via_api(self, year: int, week: int) -> list:
        """
        Use the /api/v3/timetables endpoint (unauthenticated, airType=all).
        Times are requested in UTC; parse_dt() converts to TZ later.
        """
        params = {"week": week, "year": year, "airType": "all", "tz": "UTC"}
        try:
            r = await self.client.get(self._API_URL, params=params)
            if r.status_code == 200:
                data    = r.json()
                ...
                if entries:
                    return entries
                logger.warning("API returned 200 but 0 usable entries")
            elif r.status_code in (401, 403):
                logger.warning(
                    "API returned %d — will try HTML fallback", r.status_code
                )
            else:
                logger.warning("API returned unexpected status %d", r.status_code)
        except Exception as exc:
    ...
    async def _fetch_via_html(self, year: int, week: int) -> list:
        ...
        url = f"{AS_BASE_URL}/?year={year}&week={week}"
        try:
            r = await self.client.get(url, headers={**self._HEADERS, "Accept": "text/html"})
            if r.status_code != 200:
                logger.warning("HTML page returned %d for %s", r.status_code, url)
                return []
            return await asyncio.get_event_loop().run_in_executor(
                None, self._parse_html, r.text
            )
        except Exception as exc:
            logger.warning("HTML fetch error: %s", exc)
            return []
    ...
    def _coerce_entry(e: dict) -> Optional[dict]:
        """
        Produce a uniform entry dict tolerant of both lower-camelCase (API)
        and PascalCase (some HTML payloads) field names.
        Returns None if the entry lacks both route and title.
        """

        def pick(*keys):
            for k in keys:
                # Try exact key, lowercase variant, and PascalCase variant
                for variant in (k, k.lower(), k[0].upper() + k[1:]):
                    v = e.get(variant)
                    if v is not None:
                        return v
        ...
        ep_date  = pick("episodeDate", "EpisodeDate", "airingAt")  or ""
        sub_date = pick("subDate",     "SubDate",     "subAiringAt") or ""
        dub_date = pick("dubDate",     "DubDate",     "dubAiringAt") or ""

        # Treat null sentinel as absent
        if ep_date  == _AS_NULL_DT: ep_date  = ""
        if sub_date == _AS_NULL_DT: sub_date = ""
        if dub_date == _AS_NULL_DT: dub_date = ""

        image_route = pick("imageVersionRoute", "ImageVersionRoute", "thumbnail") or ""
        # Build full CDN URL if we only have the route slug
