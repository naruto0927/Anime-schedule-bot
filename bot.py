...
class ScheduleProcessor:
    ...
    def process(entries: list) -> dict:
        """Process raw timetable entries into a day-keyed dict.
        Always includes raw, sub, and dub times when available.
        No filtering, no hide rules, no mode overrides.
        """
        best: dict = {}
        for e in entries:
            ...
            # Determine the primary time to assign this entry to a day
    ...
    def anime_block(a: dict) -> str:
        """Render a single anime entry block showing all available versions."""
    ...
    async def handle_callback(self, _, query: CallbackQuery):
        ...
        if data == "menu:back":
            ...
            if total_pages > 1:
                row = []
                if pg > 0:
                    row.append(InlineKeyboardButton("\u25c4 Prev", callback_data=f"day:{day}:{pg-1}"))
                row.append(InlineKeyboardButton(f"Page {pg+1}/{total_pages}", callback_data="noop"))
                if pg < total_pages - 1:
                    row.append(InlineKeyboardButton("Next \u25ba", callback_data=f"day:{day}:{pg+1}"))
                nav.append(row)
