...
class Database:
    ...
    async def all_chats_with_rem_topic(self) -> list:
        ...
        # Group 2: subscribed chats with no 'rem' topic bound.
        # Only skip chats already handled in Group 1 (has a rem topic).
        # Chats with only a 'nyaa' topic still get plain reminders delivered here.
        async for doc in self.chats.find({"subscribed": True}):
            cid = doc["chat_id"]
            if cid in seen:
                continue
