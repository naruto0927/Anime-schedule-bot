...
class Database:
    ...
    async def set_cache(self, key: str, value, ttl: int = 21600):
        exp = datetime.utcnow() + timedelta(seconds=ttl)  # naive UTC, consistent with DB
    ...
    async def get_cache(self, key: str):
        ...
        if not doc:
            return None
        # Handle both naive and aware datetimes stored in DB
        exp = doc["expires_at"]
        now = datetime.utcnow()
        if exp.tzinfo is not None:
            # Convert aware datetime to naive UTC for comparison
...
def build_filter_kb(f: dict, page: int = 1) -> InlineKeyboardMarkup:
    ...
    if page == 1:
        ...
        rows = [
            ...
            [
                InlineKeyboardButton("\u25c4 Prev",       callback_data="fltr:pg:1"),
                InlineKeyboardButton("\U0001f504 Reset",  callback_data="fltr:reset"),
                InlineKeyboardButton("\u2714\ufe0f Close", callback_data="fltr:close"),
            ],
            [InlineKeyboardButton("\u25c0 Back", callback_data="menu:back")],
...
def filter_text(f: dict, page: int = 1, mode: str = "all") -> str:
    ...
    mode_names = {"raw": "\U0001f534 Raw Only", "sub": "\U0001f535 Sub Only",
                  "dub": "\U0001f7e2 Dub Only", "all": "\U0001f31f All"}
    mode_note = (
        f"\n\u26a0\ufe0f <b>Mode override active: {mode_names.get(mode, mode)}</b>"
        f"\n<i>Air Type filter is overridden by /mode. Change with /mode all to use filters.</i>\n"
        if mode != "all" else ""
    )
...
class ReminderScheduler:
    ...
    async def build_all_jobs(self):
        ...
        for (wy, w) in week_pairs:
            entries   = await self.api.get_timetable(wy, w)
...
class AnimeBot:
    ...
    # ── Schedule helper ───────────────────────────────────────────────────
    async def _get_filters_with_mode(self, chat_id: int) -> dict:
        """Return filters dict with air_raw/sub/dub overridden by the mode setting."""
        f    = await self.db.get_filters(chat_id)
        mode = await self.db.get_mode(chat_id)
        if mode == "raw":
            f["air_raw"], f["air_sub"], f["air_dub"] = True, False, False
        elif mode == "sub":
            f["air_raw"], f["air_sub"], f["air_dub"] = False, True, False
        elif mode == "dub":
            f["air_raw"], f["air_sub"], f["air_dub"] = False, False, True
        # "all" → leave filter settings as-is
        return f

    async def _get_schedule(self, chat_id: int) -> dict:
        y, w = current_week_year()
        entries = await self.as_api.get_timetable(y, w)
        f = await self._get_filters_with_mode(chat_id)
        return ScheduleProcessor.process(entries, f)

    async def _get_schedule_week(self, year: int, week: int) -> list:
        """Raw entries for a specific year/week (for reminders)."""
        return await self.as_api.get_timetable(year, week)
    ...
    async def cmd_filter(self, _, msg: Message):
        ...
        mode = await self.db.get_mode(msg.chat.id)
        await msg.reply(filter_text(f, mode=mode), reply_markup=build_filter_kb(f),
                        parse_mode=enums.ParseMode.HTML)
    ...
    async def cmd_anime(self, _, msg: Message):
        ...
        start = f"{sd['year']}-{int(sd.get('month') or 0):02}-{int(sd.get('day') or 0):02}" if sd.get("year") else "?"
        cap   = (
    ...
    async def handle_callback(self, _, query: CallbackQuery):
        ...
        if data.startswith("fltr:"):
            ...
            # Determine current page from message text or caption
            msg_content = query.message.text or query.message.caption or ""
            cur_page = 2 if "Page 2/2" in msg_content else 1
            ...
            if action == "_":
                ...
                await self.safe_edit(query,
                    filter_text(f, pg, mode=mode), reply_markup=build_filter_kb(f, pg),
                    parse_mode=enums.ParseMode.HTML,
                )
                await query.answer()
                return
                ...
                mode = await self.db.get_mode(chat_id)
                await self.safe_edit(query,
                    filter_text(f, cur_page, mode=mode), reply_markup=build_filter_kb(f, cur_page),
                    parse_mode=enums.ParseMode.HTML,
                )
        ...
        if data == "menu:back":
            ...
            mode = await self.db.get_mode(chat_id)
            await self.safe_edit(query,
                filter_text(f, mode=mode), reply_markup=build_filter_kb(f),
                parse_mode=enums.ParseMode.HTML,
            )
