...
class NyaaScraper:
    ...
    # Fallback mirrors when nyaa.si returns 5xx (e.g. 504 Gateway Timeout)
    FALLBACK_SOURCES = {
        "varyg1001": "https://nyaa.land/user/varyg1001?page=rss&q=1080",
        "ToonsHub":  "https://nyaa.land/?page=rss&q=%5BToonsHub%5D+1080",
    }
    HEADERS = {
        "User-Agent": (
    ...
    async def _fetch_rss(self, url: str, source: str) -> list:
        urls_to_try = [url]
        fallback = self.FALLBACK_SOURCES.get(source)
        if fallback:
            urls_to_try.append(fallback)
        for try_url in urls_to_try:
            for attempt in range(2):
                try:
                    r = await self.client.get(try_url)
                    ...
                    if r.status_code >= 500:
                        logger.warning(
                            "Nyaa RSS [%s] returned %d from %s — trying fallback",
                            source, r.status_code, try_url,
                        )
                    ...
                    logger.info("Nyaa RSS [%s] fetched %d entries from %s", source, len(entries), try_url)
                    return entries
        ...
        try:
            ...
            for item in soup.find_all("item"):
                ...
                if source in ("varyg1001", "ToonsHub"):
                    ...
                    # subsplease RSS uses <enclosure url="..."> for the torrent link,
                    # same as nyaa. The <link> tag is a self-closing XML element that
                    # html.parser cannot read reliably — never use get_text() on it.
                    enc = item.find("enclosure")
                    if enc and enc.get("url"):
                        torrent_url = enc["url"]
                    else:
                        # Fallback: try link tag (only works with lxml xml parser)
                        link_tag = item.find("link")
                        torrent_url = (link_tag.get("href") or link_tag.get_text(strip=True)) if link_tag else ""
                    guid_tag    = item.find("guid")
                    uid         = guid_tag.get_text(strip=True) if guid_tag else torrent_url
                    view_url    = "https://subsplease.org/"
                    # Try to get size from <enclosure length="..."> (bytes)
                    if enc and enc.get("length"):
                        try:
                            length_b = int(enc["length"])
                            size = f"{length_b / (1024**3):.2f} GiB" if length_b > 0 else "~1.4 GiB"
