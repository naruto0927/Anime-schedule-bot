...
class NyaaScraper:
    ...
    def _parse_animetosho_rss(xml: str, uploader: str) -> list:
        """
        Parse AnimeTosho RSS2 feed for a given uploader.
        AnimeTosho search (q=uploader) already filters by uploader name in its index —
        we do NOT filter by <nyaa:submitter> here because AnimeTosho uses its own
        schema and may not expose that tag consistently.
        We only apply the 1080p title filter.
        """
    ...
    def format_entry(e: dict) -> str:
        ...
        if uploader == "subsplease":
            ...
            view_line = f"\n🔗 <a href=\"{view_url}\">View on AnimeTosho</a>"
        else:
            view_line = ""
        return (
            f"🎌 <b>{e['title']}</b>\n"
