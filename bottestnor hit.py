...
class NyaaScraper:
    ...
    async def _fetch_subsplease_rss(self) -> list:
        """
        Fetch SubsPlease 1080p feeds from subsplease.org.
        Fetches torrent feed and magnet feed concurrently, merges them so each
        entry has both a .torrent download URL and a magnet link.
        """
        async def _get(url):
            for attempt in range(3):
                try:
        ...
        torrent_xml, magnet_xml = await asyncio.gather(
            _get(self.SUBSPLEASE_TORRENT_RSS),
            _get(self.SUBSPLEASE_MAGNET_RSS),
        )
        entries = await asyncio.get_event_loop().run_in_executor(
            None, self._parse_subsplease_torrent, torrent_xml or ""
        )
        if magnet_xml:
            magnet_map = await asyncio.get_event_loop().run_in_executor(
                None, self._parse_subsplease_magnet, magnet_xml
            )
            for e in entries:
                e["magnet_url"] = magnet_map.get(e["title"], "")
        logger.info("SubsPlease RSS fetched %d entries", len(entries))
        return entries

    @staticmethod
    def _parse_subsplease_torrent(xml: str) -> list:
        """Parse SubsPlease torrent RSS."""
    ...
    @staticmethod
    def _parse_subsplease_magnet(xml: str) -> dict:
        """Parse SubsPlease magnet RSS — returns {title: magnet_uri} map."""
        result = {}
        if not xml:
            return result
        try:
            ...
            for item in soup.find_all("item"):
                ...
                magnet = ""
                if link_tag:
                    magnet = link_tag.get("href") or link_tag.get_text(strip=True) or ""
                if not magnet:
                    enc = item.find("enclosure")
                    magnet = enc["url"] if enc and enc.get("url") else ""
                if magnet:
                    result[title] = magnet
        except Exception as exc:
            logger.error("SubsPlease magnet RSS parse error: %s", exc)
        return result
